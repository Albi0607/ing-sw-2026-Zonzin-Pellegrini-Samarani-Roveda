...
public class BottomRowController {
    ...
    public void updateLower(ObservableList<CardDTO> bottomRow) {
        if(this.currentRound==gameController.getCurrentRound()){
            //non è cambiato nulla le carte sono rimaste le stesse: ritorno
            if(cards.size()==bottomRow.size()){
                refreshInteraction();
                return;
            }
            //altrimenti è stata scelta una carta che va eliminata
            for(int i = 0;i<cards.size();i++){
                if(!cards.get(i).getDTO().id.equals(bottomRow.get(i).id)){
                    bottomRowCards.getChildren().remove(i);
                    cards.remove(i);
        ...
        // svuota la riga
        bottomRowCards.getChildren().clear();
        cards.clear();

        int position = 0;

        // crea una carta per ogni CardDTO
        for (CardDTO dto : bottomRow) {
            ...
            try {
                ...
                bottomRowCards.getChildren().add(cardNode);
                position++;

            } catch (IOException e) {
                System.out.println("ERRORE DI CARICAMENTO DELLE CARTE NEL BOTTOMROWCARD: " + e.getMessage());
                e.printStackTrace();
            }
